...
class TitleTopicClassifier(object):
    def __init__(self):
        self.dictionary = corpora.Dictionary.load_from_text(
            'gensim_title.dict')
        with open('clf_title.pickle', 'rb') as f:
...
if __name__ == '__main__':
    ...
    if not args.cache:
        ...
        dictionary.save_as_text('gensim_title.dict')
        ...
        with open('clf_title.pickle', 'wb') as f:
            pickle.dump(clf, f)
        print("Classifier making finish", file=sys.stderr)

    else:
        dictionary = corpora.Dictionary.load_from_text('gensim_title.dict')
        with open('clf_title.pickle', 'rb') as f:
            clf = pickle.load(f)
